...
Gemini critic (set `critic_override: gemini` on the task) when the diff
exceeds ~400 lines, touches more than 6 files, the acceptance criteria
are ambiguous, or you are uncertain about correctness. The configured
default lives in `.orch/config/orchestrator.toml` under `[critic] mode`.
